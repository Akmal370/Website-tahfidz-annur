 ...
 * @version         3.0.0
 */
abstract class REST_Controller extends MY_Admin {
    ...
    public function __construct($config = 'rest')
    {
        ...
        $supported_formats = $this->_supported_formats;
...
//        $this->classes_m->setBranch($output_data['userdata']->schoolID);
//        $this->student_m->setBranch($output_data['userdata']->schoolID);
//        $this->section_m->setBranch($output_data['userdata']->schoolID);
//        $this->teacher_m->setBranch($output_data['userdata']->schoolID);
//        $this->parents_m->setBranch($output_data['userdata']->schoolID);
//        $this->setting_m->setBranch($output_data['userdata']->schoolID);
//        $this->idmanager_m->setBranch($output_data['userdata']->schoolID);
//        $this->subject_m->setBranch($output_data['userdata']->schoolID);
//        $this->syllabus_m->setBranch($output_data['userdata']->schoolID);
//        $this->studentrelation_m->setBranch($output_data['userdata']->schoolID);
//        $this->parents_info_m->setBranch($output_data['userdata']->schoolID);
//        $this->student_info_m->setBranch($output_data['userdata']->schoolID);
//    }
    /**
     * De-constructor
     *
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        $this->object_called = $object_called;
...
//                }
    ...
    protected function jwt_decode($token)
    {
        try{
            ...
            JWT::$leeway = 720000;
            $decoded = (array) JWT::decode($token, $this->jwt_secret_key, ['HS256']);
            $decoded['iat'] = time();
            $decoded['exp'] = time() + 720000;

            if (!empty($decoded)) {
                return $this->jwt_decode($this->jwt_encode($decoded));
            }
        } catch ( \Exception $e ){
            $this->response([
                $this->config->item('rest_status_field_name') => 'invalid_token',
     ...
     * @return string
     */
    public function jwt_refresh($token)
    {
        JWT::$leeway = 720000;
        $decoded = (array) JWT::decode($token, $this->jwt_secret_key, ['HS256']);
        $decoded['iat'] = time();
        ...
        if (!empty($decoded)) {
            return $this->jwt_encode($decoded);
        }
