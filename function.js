...
function hapus_data(e, id, url = '', text = '',reload = false) {
    ...
    if (text) {
        ...
        var message = 'Apakah anda yakin akan menghapus data ini? Data yang dihapus tidak bisa di pulihkan';
    }
                    ...
                    if (data.status == 200 || data.status == true) {
                        if (data.alert) {
                            if (reload == 'big') {
                                if (data.alert) {
                                    sessionStorage.setItem('isReload', 'true');
                                    sessionStorage.setItem('alert_icon', data.alert.icon);
                                    sessionStorage.setItem('alert_message', data.alert.message);
                                }
...
function switching(element, e, id) {
    ...
    if ($(element).is(':checked')) {
        var value = 'Y';
        var type = false;
        var message = ucfirst('apakah Anda yakin ingin membuka akses ke data ini? Kemudian data tersebut akan dapat diakses oleh sistem');
    } else {
        var value = 'N';
         var type = "textarea";
        var message = ucfirst('apakah Anda yakin ingin menutup akses ke data ini? data tidak akan dapat mengakses sistem');
    }
    Swal.fire({
        text: message,
        icon: icon,
        ...
        inputPlaceholder: 'Sertakan alasan mematikan data',
        showCancelButton: true,
        buttonsStyling: !1,
        confirmButtonText: 'Lanjutkan',
        cancelButtonText: 'Batal',
